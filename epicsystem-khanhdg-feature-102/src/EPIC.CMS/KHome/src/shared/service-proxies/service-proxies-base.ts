...
export class ServiceProxyBase {
	...
	public requestGet(url: String): Observable<any> {
		...
		const listAPi = AppConsts.listAPIUseTradingProviderIds;
        const tradingProviderIds = AppConsts.tradingProviderIds;
		if (tradingProviderIds?.length > 0){
			url_ = this.baseUrl + url + '?';
			listAPi.forEach(api => {
				if (url_.includes(api)){
	 ...
	 * @returns
	 */
	public requestDownloadFile(url: String, isExportReport?: boolean): Observable<any> {
		let url_ = this.baseUrl + url;
		url_ = url_.replace(/[?&]$/, "");
			...
			return this.http
				...
				.pipe(
					_observableMergeMap(async (response_: any) => {
						...
						// Xuất báo cáo nếu xử lý bình thường bị lỗi nên phải tách ra
						if (isExportReport){
	 ...
	 * @returns
	 */
	public requestDelete(url: String, body?: any): Observable<any> {
		let url_ = this.baseUrl + url;
		url_ = url_.replace(/[?&]$/, "");
	...
	public viewFilePDFFromAPI(url: String) {
		let url_ = this.baseUrl + url;
		url_ = url_.replace(/[?&]$/, '');
		...
		return this.http.get(url_, options_).pipe(
		  	_observableMergeMap(async (response_: any) => {
				var file = new Blob([response_.body], { type: 'application/pdf' });
